...
elif [ -d "$PROJECT_DIR/demo-site-template" ]; then
  echo "No demo-site/ yet in main worktree; bootstrapping from demo-site-template/..." >&2
  (cd "$WORKTREE_PATH" && bash scripts/bootstrap-demo-site.sh) >&2
fi
