...
const ListFieldTypes = {
  ...
  outputSchema: getFieldTypeResponse,
  slices: ["list"],
  annotations: {
    readOnlyHint: true,
  },
  handler: async () => {
    return executeGetApiCall<ReturnType<ApiClient["getFieldType"]>, ApiClient>(
      (client) => client.getFieldType(CAPTURE_RAW_HTTP_RESPONSE)
    );
  },
} satisfies ToolDefinition<typeof emptyInput.shape, typeof getFieldTypeResponse>;
