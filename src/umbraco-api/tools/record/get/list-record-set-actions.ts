...
const ListRecordSetActionsTool = {
  ...
  outputSchema: getRecordSetActionsResponse,
  slices: ["list"],
  annotations: {
    readOnlyHint: true,
  },
  handler: async () => {
    return executeGetApiCall<ReturnType<ApiClient["getRecordSetActions"]>, ApiClient>(
      (client) => client.getRecordSetActions(CAPTURE_RAW_HTTP_RESPONSE)
    );
  },
} satisfies ToolDefinition<typeof emptyInput.shape, typeof getRecordSetActionsResponse>;
