...
1. First, list all data source types using list-data-source-types to find a valid type ID. Pick the first type from the results and note its required settings.
2. Generate a unique timestamp identifier (e.g., current time in milliseconds).
3. Create a new data source with name "TestDataSource-{timestamp}" using the type ID from step 1. Provide placeholder values for any required settings (e.g. for a SQL Database type, provide settings like {"Connection": "Server=localhost;Database=test;", "Table": "TestTable"}). Save the ID returned from the creation.
